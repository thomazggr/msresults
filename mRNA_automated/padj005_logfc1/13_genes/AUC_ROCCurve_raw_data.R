...
except_get_expression <- function(gse, annot_gpl, gpl, samples, annotation_data, gene_names_filter, labels){
    ...
    exp_data_filtered <- dplyr::select(exp_data_filtered, -c(illumina.x, illumina.y, entrez_id, ext_gene, affy)) %>%
                            t() %>%
                            as.data.frame()
...
common_get_expression <- function(gse, annot_gpl, gpl, samples, annotation_data, gene_names_filter, labels){
    ...
    row.names(exp_data_filtered) <- exp_data_filtered$ext_gene
    exp_data_filtered <- dplyr::select(exp_data_filtered, -c(illumina, entrez_id, ext_gene, affy)) %>%
                            t() %>%
                            as.data.frame()
...
for(d in datasets){
    if(d[[1]] == "GSE37031"){
        data <- except_get_expression(d[[1]], d[[2]], d[[3]], d[[4]], ann, gene_names_filter, d[[5]])
    } else {
        data <- common_get_expression(d[[1]], d[[2]], d[[3]], d[[4]], ann, gene_names_filter, d[[5]])
    }
