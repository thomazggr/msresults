...
def generate_list(path, skip=False, keepid=False, usemircol=False):
    mir = "hsa-miR-200b|483|hsa-miR-155|hsa-miR-940|hsa-miR-222|hsa-miR-221|181d|hsa-miR-200a|hsa-miR-193b|hsa-miR-31|hsa-miR-143|199b|378|hsa-miR-150|hsa-miR-182|hsa-miR-10a|hsa-miR-21|146b|200c|hsa-miR-1234"
    ...
    if usemircol == True:
        ...
        frames = [
            filtra_hsa[filtra_hsa.id.str.contains(mir)].query(
                "pval <= 0.01 & logfc < -1"
            ),
            filtra_hsa[filtra_hsa.id.str.contains(mir)].query(
                "pval <= 0.01 & logfc > 1"
        ...
        # filtra_p = filtra_hsa[filtra_hsa.id.str.contains(mir)].query(
        #    "pval <= 0.01 & logfc < -1 | logfc > 1"
        # )
        lista = list(filtra_p["id"])
        return lista
...
"""lista10694, mirs = generate_list(
    path="datasets/GSE10694/GSE10694_results.txt",
    skip=True,
    keepid=True,
    usemircol=True,
)"""
...
def run_heatmaps(dataset):
    ...
    restable = valores[valores["ID_REF"].str.contains("|".join(lista))]
    # if dataset == "GSE10694":
    ...
    print(dataset)
    print(lista)
    pathtocsv = "csv/" + dataset + "_resultadonew.csv"
